            ...
            $(function () {
                ...
                var sortactive = $(".draggable-menu").sortable({
                    ...
                    onDrop: function ($item, container, _super) {
                        ...
                        $.post("{{route('AdminFrontendMenuManagementControllerPostSaveMenu')}}", {menus: jsonString, isActive: isActive}, function (resp) {
                                ...
                                <div class='{{$menu->is_dashboard?"is-dashboard":""}}' title="{{$menu->is_dashboard?'This is setted as Dashboard':''}}">
                                    <i class='{{($menu->is_dashboard)?"icon-is-dashboard fa fa-dashboard":$menu->icon}}'></i> {{$menu->name}} <span
                                            class='pull-right'><a class='fa fa-pencil' title='Edit'
                                                                  href='{{route("AdminFrontendMenuManagementControllerGetEdit",["id"=>$menu->id])}}?return_url={{urlencode(Request::fullUrl())}}'></a>&nbsp;&nbsp;<a
                                                title='Delete' class='fa fa-trash'
                                                onclick='{{CRUDBooster::deleteConfirm(route("AdminFrontendMenuManagementControllerGetDelete",["id"=>$menu->id]))}}'
                                            ...
                                            <li data-id='{{$child->id}}' data-name='{{$child->name}}'>
                                                <div class='{{$child->is_dashboard?"is-dashboard":""}}'
                                                    ...
                                                    <span class='pull-right'><a class='fa fa-pencil' title='Edit'
                                                                                href='{{route("AdminFrontendMenuManagementControllerGetEdit",["id"=>$child->id])}}?return_url={{urlencode(Request::fullUrl())}}'></a>&nbsp;&nbsp;<a
                                                                title="Delete" class='fa fa-trash'
                                                                onclick='{{CRUDBooster::deleteConfirm(route("AdminFrontendMenuManagementControllerGetDelete",["id"=>$child->id]))}}'
                                                                href='javascript:void(0)'></a></span>
                                                    {{--<br/><em class="text-muted">--}}
                                                        {{--<small><i class="fa fa-users"></i> &nbsp; {{implode(', ',$privileges)}}</small>--}}
                                                    {{--</em>--}}
