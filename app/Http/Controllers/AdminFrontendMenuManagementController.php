<?php namespace App\Http\Controllers;
	...
	class AdminFrontendMenuManagementController extends \crocodicstudio\crudbooster\controllers\CBController {
...
//                'dataenum' => ['Module', 'Statistic', 'URL', 'Controller & Method', 'Route'],
                'dataenum' => ['URL', 'Controller & Method', 'Route'],
...
//            $this->form[] = ["label" => "id_cms_privileges", "name" => "id_cms_privileges", "type" => "hidden", "value" => $id_cms_privileges];
        ...
        public function getIndex()
        {
            ...
            $menu_active = DB::table($this->table)->where('parent_id', 0)->where('is_active', 1)->orderby('sorting', 'asc')->get();

            foreach ($menu_active as &$menu) {
                $child = DB::table($this->table)->where('is_active', 1)->where('parent_id', $menu->id)->orderby('sorting', 'asc')->get();
                if (count($child)) {
                    $menu->children = $child;
                }
            }

            $menu_inactive = DB::table($this->table)->where('parent_id', 0)->where('is_active', 0)->orderby('sorting', 'asc')->get();
            ...
            $page_title = 'Front Menu Management';

            return view('frontend_menus_management', compact('menu_active', 'menu_inactive', 'privileges', 'id_cms_privileges', 'return_url', 'page_title'));
        }
        ...
        public function hook_before_add(&$postdata)
        {
            ...
            if ($postdata['is_dashboard'] == 1) {
                ...
                //DB::table($this->table)->where('id_cms_privileges', $postdata['id_cms_privileges'])->where('is_dashboard', 1)->update(['is_dashboard' => 0]);
